...
from LanguageModeling.data_collator import (
    DataCollatorForFormLMContinualPretrain,
)
from models import FormLMConfig, FormLMForConditionalGeneration, FormLMTokenizer, FormLinearize
...
def main():
    ...
    # Load the datasets from existed files.
    input_column = "form"
    ...
    if training_args.do_train:
        ...
        for _, _, files in os.walk(os.path.join(data_args.data_root_dir, "train")):
            for file in files:
                ...
                form = form_linearizer.linearize_form(data)
                train_dataset[input_column].append(form)
    ...
    if training_args.do_eval:
        ...
        for _, _, files in os.walk(os.path.join(data_args.data_root_dir, "dev")):
            for file in files:
                ...
                form = form_linearizer.linearize_form(data)
                validation_dataset[input_column].append(form)
    ...
    if training_args.do_predict:
        ...
        for _, _, files in os.walk(os.path.join(data_args.data_root_dir, "test")):
            for file in files:
                ...
                form = form_linearizer.linearize_form(data)
                test_dataset[input_column].append(form)
    ...
    def preprocess_function(examples):
        # The corpus we use has 'title', 'description' and 'blocks'.
        # We concatenate 'title', 'description' and 'blocks' to form the inputs. Blocks are separated by
        # tokenizer.sep_token.
    ...
    # Data collator
    data_collator = DataCollatorForFormLMContinualPretrain(
        tokenizer=tokenizer,
        model=model,
        span_mlm=True,
